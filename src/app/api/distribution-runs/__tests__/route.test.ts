...
describe('GET /api/distribution-runs', () => {
  ...
  it('reads Neon and nests per-engine QoE on each run', async () => {
    ...
    const sql = jest.fn()
      ...
      .mockResolvedValueOnce([
        { run_id: 2, player_engine: 'gpac', samples: 9000, startup_p50: 820, startup_p95: 1500, startup_p99: 2100, rebuffer_avg: 0.02, bitrate_avg: 2800, abr_switches_avg: 3.2, stalls_avg: 0.4 },
        { run_id: 2, player_engine: 'shaka', samples: 20, startup_p50: 760, startup_p95: 1100, startup_p99: 1400, rebuffer_avg: 0.01, bitrate_avg: 3000, abr_switches_avg: 2.0, stalls_avg: 0.1 },
      ])
      .mockResolvedValueOnce([
    ...
    expect(t2.qoe[0].ABRSwitchesAvg).toBe(3.2);
    expect(t2.qoe[0].StallsAvg).toBe(0.4);
